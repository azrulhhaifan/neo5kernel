...
//because pcb_version.h   Function :get_pcb_version had been removed
...
#define APQ_NFC_VEN_GPIO 91 //NFC_ENABLE
#define APQ_NFC_FIRM_GPIO 92  //NFC_UPDATE
#define APQ_NFC_IRQ_GPIO 21   //NFC_IRQ
...
struct pn544_dev	
{
	...
	struct regulator * pvdd;//yuyi add
...
printk("pn544_dev_ioctl enter cmd = %d,arg = %ld\n",cmd,arg);
	switch (cmd) 
	{
	case PN544_SET_PWR:
		if (arg == 2) 
...
/*OPPO yuyi 2014-02-24 add begin for nfc pn65T*/
	...
	pvdd= devm_regulator_get(&(client->dev), "nfc_1v8");
	if (IS_ERR(pvdd)) {
		ret = PTR_ERR(pvdd);
		dev_err(&client->dev,
			"Regulator get failed pvdd ret=%d\n", ret);
			return ret;
	}	
	ret = regulator_set_voltage(pvdd, 1800000,
				1800000);
	if (ret) {
		dev_err(&client->dev,
			"Regulator pvdd failed pvdd ret=%d\n", ret);
			return ret;
	}		
    /***enable the 1v8 power*****/
	ret = regulator_enable(pvdd);
	if (ret) {
		dev_err(&client->dev,
			"Regulator pvdd enable failed ret=%d\n", ret);
			return ret;
	}
	/***should enable the 1v8 power*****/
	msleep(10);
	printk("pn544_power_init power end\n");	
...
/*OPPO yuyi 2014-02-24 add begin for nfc pn65T*/
		...
		p65T_clk  = clk_get(&client->dev, "bbclk2");
		if (p65T_clk == NULL) {
